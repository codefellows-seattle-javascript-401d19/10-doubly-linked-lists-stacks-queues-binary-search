...
class Stack {
  ...
  //Declare push method that takes in the node as parameter
  push(item) {
    //Call addNodeAsFirst method from linked list class on linkedStack which will add it as the top node in the stack.
    this.linkedStack.addNodeAsFirst(item);
  }

  //Declare pop method
  pop() {
    if (!this.linkedStack.listLength) {
      return;
    }
    //If stack has at least one node, determine its value
    let val = this.linkedStack.head.value;
    //Call removeFirstNode method from linked list class on linkedStack which will remove the top node on the stack.
    this.linkedStack.removeFirstNode();

    return val;
  }

  peek() {
    if (!this.linkedStack.head) { return; }
    return this.linkedStack.head.value;
  }

  get listLength() {
    return this.linkedStack.listLength;
  }

  displayStackObj(log) {
    this.linkedStack.displayStackObj(log);
  }
}
